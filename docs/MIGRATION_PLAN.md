...
> 📦 **Статус:** документ — **исторический**. Этап миграции из локального зеркала `/mirror/` в `src/content/` завершён 30.04.2026. Директория `/mirror/` удалена из репозитория; дальнейшая сверка контента ведётся напрямую с живым WordPress (`https://sit-salsk.ru/`) через скрипт `node sync-check.mjs <URL> --follow-posts`.
>
> Документ оставлен как контекст архитектурных решений миграции и описание алгоритма HTML → Markdown.
