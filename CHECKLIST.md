...
### 7.0 Перенос файлов на хостинг (`docs/`) — план миграции

**Цель.** Освободить ~425 МБ в `src/assets/uploads/` (и столько же дублей в `public/assets/uploads/` после сборки), вынеся PDF/DOC/ZIP и архивы на боевой хостинг в папку `docs/`. Контентные изображения (фото руководителей, преподавателей, баннеры, иконки разделов) остаются в проекте — они нужны при сборке шаблонов и для локального превью.
...
| Базовый URL для документов | `https://xn----8sbwke6acce8h.xn--p1ai/docs/` |
| Структура на хостинге | зеркалит `src/assets/uploads/<folder>/` → `docs/<folder>/` |
| Переменная в коде | `site.docsBaseUrl` в `src/_data/site.js` (с возможностью переопределения через env `DOCS_BASE_URL`) |
| Использование в шаблонах | `{{ site.docsBaseUrl }}<folder>/имя-файла.pdf` или фильтр `\| docs` |
| Резервная совместимость | 301-редиректы `/assets/uploads/<folder>/*` → `/docs/<folder>/*` на хостинге (`.htaccess`) — настраивается после полного переноса |

**Карта переноса (10 подпапок, ~425 МБ, 530 файлов):**

| Подпапка | Связь с разделом | Файлов | Размер | URL после переноса | Статус |
|----------|------------------|-------:|-------:|--------------------|:------:|
| `dokumenty/` | 7.2.3 Документы + 7.2.4 Противодействие коррупции | 304 | 322 МБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/dokumenty/` | ❌ |
| `vsoko/` | 7.2.3.1 ВСОКО | 90 | 84 МБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/vsoko/` | ❌ |
| `struktura/` | 7.2.2 Структура и органы управления | 33 | 9.7 МБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/struktura/` | ❌ |
| `pedagog/` | 7.2.8.1 Педагогический состав | 16 | 3.6 МБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/pedagog/` | ❌ |
| `priemnaya-kampaniya-2026/` | 7.1.3 Приёмная кампания 2026 | 15 | 2.4 МБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/priemnaya-kampaniya-2026/` | ❌ |
| `mezhdunarodnoe/` | 7.2.7 Международное сотрудничество | 9 | 1.6 МБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/mezhdunarodnoe/` | ❌ |
| `podacha-elektronnaya-pochta/` | 7.1.6 + 7.1.7 Подача документов (бланки заявлений) | 6 | 336 КБ | `https://xn----8sbwke6acce8h.xn--p1ai/docs/podacha-elektronnaya-pochta/` | ❌ |
| `rukovodstvo/` | 7.2.8 Руководство — фото руководителей | 9 | 1008 КБ | — | 🔒 оставить в проекте (контентные изображения) |
| `virtualnaya-ekskursiya/` | 7.1.5 Виртуальная экскурсия — 46 фото галереи | 46 | 768 КБ | — | 🔒 оставить в проекте (по умолчанию); обсудить отдельно при необходимости |
| `kontakty-grafik/` | 7.1.9 Контакты и график — баннер | 1 | 24 КБ | — | 🔒 оставить в проекте (баннер) |

**Итого к переносу:** 7 подпапок, **473 файла**, **~423 МБ**. Остаётся в проекте: 56 изображений, ~1.8 МБ.

---

**Подготовительные шаги (выполняются один раз до начала переноса):**

| № | Шаг | Статус |
|---|-----|:------:|
| П1 | Создать `src/assets/uploads.zip` (резервный архив всех загрузок) и передать пользователю на скачивание | ❌ |
| П2 | Создать `src/_data/site.js` с переменной `docsBaseUrl` (Punycode + env override `DOCS_BASE_URL`) | ❌ |
| П3 | Зарегистрировать Nunjucks-фильтр `\| docs` в `.eleventy.js` для удобной подстановки в шаблонах | ❌ |
| П4 | Проверить, что хостинг отдаёт файлы из `docs/` с корректным `Content-Type` для PDF/DOC/ZIP (загрузить тестовый файл и проверить `curl -I`) | ❌ |
| П5 | По мере переноса каждой подпапки — снимать passthrough copy в `.eleventy.js` (или удалять физически после замены ссылок) | по подпапкам |
| П6 | После полного переноса — добавить 301-редиректы `/assets/uploads/*` → `/docs/*` в `.htaccess` хостинга | ❌ |

---

**Универсальная процедура переноса одной подпапки (повторяется для каждой строки карты):**

| Шаг | Действие | Кто делает |
|-----|----------|------------|
| 1 | Проверить состав файлов: `ls -la src/assets/uploads/<folder>/`, выгрузить точный список в текстовый файл | агент |
| 2 | Залить подпапку на хостинг через FTP/SFTP в `docs/<folder>/` с сохранением имён файлов и подпапок | пользователь |
| 3 | Проверить доступность 2–3 случайных файлов: `curl -I https://сит-сальск.рф/docs/<folder>/<file>` → ожидаем `HTTP/2 200` | пользователь сообщает агенту |
| 4 | Найти все ссылки на `/assets/uploads/<folder>/` в `src/content/`, `src/_data/`, `src/_includes/`: `rg "/assets/uploads/<folder>/" src` | агент |
| 5 | Заменить относительные ссылки на `{{ site.docsBaseUrl }}<folder>/...` (или фильтр `\| docs`) во всех найденных местах | агент |
| 6 | Локальная сборка `npm run build`, проверить страницы раздела на `HTTP 200` и работу ссылок (несколько случайных) | агент |
| 7 | Удалить `src/assets/uploads/<folder>/` из проекта (`git rm -r`) | агент |
| 8 | Закоммитить (`feat(docs): migrate <folder> to hosting`) и запушить | агент (автопуш) |
| 9 | Проставить ✅ в карте переноса 7.0 и в соответствующем подразделе 7.X | агент |

---

**Принципы:**

- Перенос идёт **по одной подпапке за раз**, начиная с самой тяжёлой (`dokumenty/` 322 МБ → `vsoko/` 84 МБ → дальше по убыванию). После каждой подпапки — коммит, проверка, отчёт пользователю.
- Никаких удалений из проекта **до подтверждения**, что файлы доступны на хостинге.
- Резервный архив `uploads.zip` создаётся **до** удаления первой подпапки.
- `mirror/` (101 МБ) и `_mentor/` (12 МБ) остаются в проекте — это исходники миграции с WordPress, использующиеся как референс при наполнении разделов.
